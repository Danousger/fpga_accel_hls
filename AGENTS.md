...
- **Block Design** — two scripts, both RTL-direct (neither uses IP catalog):
  ...
  - `vivado/build_bd_systolic.tcl` — **working** systolic flow. Adds HLS RTL
    ...
    (no board preset; DDR3 `MT41K256M16 RE-125`, matching the board's 1 GiB
    memory geometry). Runs synth+impl+write_bitstream
  ...
  - **DDR3 PARTNO matters**: use `MT41K256M16 RE-125`. The previously selected
    J part made the PS7 configuration expose only 512 MiB while the Pynq image
    and physical board use 1 GiB. DMA buffers allocated above that boundary
    then appeared as an AXIS hang because the DMA could not access them. Also
...
- `hls/systolic_csim/` and `hls/build/systolic_single/`
- any `**/solution*/` — contains generated RTL under `syn/verilog/`, `syn/vhdl/`

Source of truth: `hls/src/matmul_systolic_*.cpp`. Vitis HLS 2020.2 synthesis
also requires the maintained flattened copy `hls/build/matmul_systolic_all.cpp`.

## PS-side reference

There is no board-validated Python driver for the current three-DMA systolic
design. `ps_driver/notebooks/axis_test.ipynb` targets the current `design_1.bit`
and its A/B MM2S plus C S2MM topology. It uses one 16x16 Q8.8 tile and bounded
status polling. Do not claim it validates the design until it passes on a
freshly generated bitstream.
